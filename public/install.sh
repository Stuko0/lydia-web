#!/bin/bash
# ============================================================================
# Lydia Agent Installer
# ============================================================================
# Installation script for Linux, macOS, and Android/Termux.
# Uses uv for desktop/server installs and Python's stdlib venv + pip on Termux.
#
# Usage:
#   curl -fsSL https://lydia-agent.stuko.dev/install.sh | bash
#
# Or with options:
...
# Python-driven tool session (e.g. Lydia terminal tool). A pre-set PYTHONPATH
...
LYDIA_HOME="${LYDIA_HOME:-$HOME/.lydia}"
...
#   code at /usr/local/lib/lydia-agent, command at /usr/local/bin/lydia,
#   data still at /root/.lydia (LYDIA_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   'lydia update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Lydia.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.lydia/lydia-agent"
            echo "                   default (root, Linux): /usr/local/lib/lydia-agent"
            echo "  --lydia-home PATH  Data directory (default: ~/.lydia, or \$LYDIA_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Lydia installs the code under"
            echo "  /usr/local/lib/lydia-agent and links the command into"
            echo "  /usr/local/bin/lydia (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$LYDIA_HOME"
            echo "  (default /root/.lydia).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$LYDIA_HOME/lydia-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs lydia setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             🌹 Lydia Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Stuko.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `lydia update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Lydia-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Lydia Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install lydia command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `lydia` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $LYDIA_HOME/lydia-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $LYDIA_HOME/lydia-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/lydia-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $LYDIA_HOME/lydia-agent — then preserve it)
#
...
resolve_install_layout() {
    ...
    if is_termux; then
        INSTALL_DIR="$LYDIA_HOME/lydia-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$LYDIA_HOME/lydia-agent/.git" ]; then
            INSTALL_DIR="$LYDIA_HOME/lydia-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/lydia-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/lydia-agent"
        ...
        # /usr/local/bin/lydia wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/lydia"
...
# Point a Lydia-managed Node's `npm install -g` at a directory that is on
...
# Lydia's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Lydia-managed npm, so calling it on
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://lydia-agent.stuko.dev/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Lydia owns its own uv at $LYDIA_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (lydia_cli/managed_uv.py) looks in the same
    # place, so install.sh and `lydia update` stay in sync.
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/lydia-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/lydia-uv-installer.$$.sh")"
...
# string clears that floor; anything below it is replaced with the Lydia-
...
check_node() {
    ...
    # Repair pre-existing Lydia-managed installs where `npm install -g` lands
    ...
    if [ -x "$LYDIA_HOME/node/bin/node" ] && node_satisfies_build "$("$LYDIA_HOME/node/bin/node" --version)"; then
        ...
        log_success "Node.js $("$LYDIA_HOME/node/bin/node" --version) found (Lydia-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Lydia-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.lydia/node/..."
    ...
    # Place into ~/.lydia/node/ and symlink binaries into the same bin dir
    # the lydia command uses (get_command_link_dir): /usr/local/bin for root
    ...
    log_success "Node.js $installed_ver installed to ~/.lydia/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Lydia install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Lydia Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `lydia update` path
                # (#4735).
                ...
                local stash_name
                stash_name="lydia-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``lydia update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Lydia behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up lydia command..."

    if [ "$USE_VENV" = true ]; then
        LYDIA_BIN="$INSTALL_DIR/venv/bin/lydia"
    else
        LYDIA_BIN="$(which lydia 2>/dev/null || echo "")"
        if [ -z "$LYDIA_BIN" ]; then
            log_warn "lydia not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$LYDIA_BIN" ]; then
        log_warn "lydia entry point not found at $LYDIA_BIN"
    ...
    # Create a user-facing shim for the lydia command.
    ...
    rm -f "$command_link_dir/lydia"
    cat > "$command_link_dir/lydia" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$LYDIA_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/lydia"
    log_success "Installed lydia launcher → $command_link_display_dir/lydia"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "lydia command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v lydia' \
                >/dev/null 2>&1; then
        ...
        log_info "lydia not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Lydia Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Lydia Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Lydia Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so lydia works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.lydia directory structure (config at top level, code in subdir)
    mkdir -p "$LYDIA_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.lydia/.env (top level, easy to find)
    if [ ! -f "$LYDIA_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$LYDIA_HOME/.env"
            log_success "Created ~/.lydia/.env from template"
        else
            touch "$LYDIA_HOME/.env"
            log_success "Created ~/.lydia/.env"
        fi
    else
        log_info "~/.lydia/.env already exists, keeping it"
    fi
    ...
    # Create config.yaml at ~/.lydia/config.yaml (top level, easy to find)
    if [ ! -f "$LYDIA_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$LYDIA_HOME/config.yaml"
            log_success "Created ~/.lydia/config.yaml from template"
        fi
    else
        log_info "~/.lydia/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in lydia_cli/default_soul.py — the
...
SOUL_EOF
        log_success "Created ~/.lydia/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.lydia/"

    # Seed bundled skills into ~/.lydia/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `lydia update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'lydia update'." \
        ...
        log_info "  Future 'lydia update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.lydia/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.lydia/skills/"
        else
            ...
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$LYDIA_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                ...
                log_success "Skills copied to ~/.lydia/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Lydia Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Lydia will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Lydia Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (lydia --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `lydia update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'lydia setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m lydia_cli.main setup < /dev/tty
    else
        python -m lydia_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Lydia to send/receive messages."
    ...
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            log_info "Running 'lydia whatsapp' to pair via QR code..."
            echo ""
            ...
            log_info "WhatsApp pairing skipped (non-interactive). Run 'lydia whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'lydia gateway install' later."
        return 0
    fi
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $LYDIA_CMD gateway install 2>/dev/null; then
                ...
                if $LYDIA_CMD gateway start 2>/dev/null; then
                    ...
                    log_warn "Service installed but failed to start. Try: lydia gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: lydia gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.lydia/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: lydia gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: lydia gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${GREEN}lydia${NC}              Start chatting"
    echo -e "   ${GREEN}lydia setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}lydia config${NC}       View/edit configuration"
    echo -e "   ${GREEN}lydia config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}lydia gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}lydia update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'lydia' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'lydia' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'lydia' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Lydia for pip install"
    ...
    LYDIA_CMD="$(command -v lydia 2>/dev/null || echo "")"
    if [ -n "$LYDIA_CMD" ]; then
        log_info "Running lydia setup..."
        "$LYDIA_CMD" setup
    else
        log_warn "lydia command not found on PATH"
        log_info "Try: python -m lydia_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`Lydia` rename
...
# _purge_electron_build_cache() used by `lydia desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Lydia-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `lydia desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Lydia" ]; then
            app="$desktop_dir/release/linux-unpacked/Lydia"
        elif [ -x "$desktop_dir/release/linux-unpacked/lydia" ]; then
            app="$desktop_dir/release/linux-unpacked/lydia"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Lydia.app" \
            "$desktop_dir/release/mac/Lydia.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Lydia is damaged and can't be opened").
    ...
    # checkout stays clean for the next `lydia update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Lydia-managed Node
        ...
        complete)
            ...
            # 'lydia update' on this host install. See detect_install_method().
...
main() {
    ...
    # stamp and wrongly blocks 'lydia update' on this host install.
